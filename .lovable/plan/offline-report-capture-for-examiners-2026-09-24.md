...
3. Recordings for that candidate are attached and held on the device. The examiner presses **Save** to finish a report and move straight on to the next examinee; a saved report can be reopened any time while the analysis and finding are completed. Only once the finding is made and the examiner has reviewed the report can it be **published for peer review** — and publishing is blocked until that candidate's PF folder and ESS report are attached. In a batch, each report publishes on its own, but the batch as a whole is only released once the last report in the batch has been published.
4. Working offline, a published report shows "Published (waiting to upload)". Saved-but-unfinished reports show as drafts.
5. When signal returns, the app uploads the report and recordings by itself, one file at a time, and once uploaded a green tick appears next to the submission. If a file fails it retries; nothing is lost and nothing needs re-typing.
