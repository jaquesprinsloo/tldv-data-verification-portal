-- Fix approve_polygraph_candidate to not create duplicate employees
-- Check if candidate already has an employee_id linked

CREATE OR REPLACE FUNCTION public.approve_polygraph_candidate(_candidate_id uuid)
 RETURNS TABLE(employee_id uuid, employee_number text, email text, token text, otp text, first_name text, last_name text)
 LANGUAGE plpgsql
 SECURITY DEFINER
