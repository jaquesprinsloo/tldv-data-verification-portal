        ...
        messages: [
          {
            role: "system",
            content:
              "You extract per-candidate records from a supplier vetting report. Each candidate has a 'VERIFICATION OF ID NUMBER' block AND typically a 'RISK ASSESSMENT CHECK' block (or similarly-named section) that states either 'No further Investigation Required' or 'Further Investigation Required'. The ID Number may be MASKED (e.g. '981201XXXXXXX') — return exactly what appears. Respond with ONLY a JSON object of shape:\n{\"records\":[{\"id_number\":\"981201XXXXXXX\",\"id_prefix\":\"981201\",\"status\":\"Confirmed\",\"first_names\":\"NIKESH\",\"initials\":\"N\",\"surname\":\"DHEEPLALL\",\"date_of_birth\":\"1998-12-01\",\"age\":\"27\",\"gender\":\"MALE\",\"citizenship\":\"SOUTH AFRICAN\",\"dead_alive\":\"Alive\",\"risk_assessment\":\"No further Investigation Required\"}]}\nRules: id_prefix = the first 6 digits of the ID (digits only). status = the ID verification confirmation signal (e.g. 'Confirmed', 'Not Confirmed', 'Completed'). risk_assessment = the exact outcome text from that candidate's Risk Assessment Check section (typically 'No further Investigation Required' or 'Further Investigation Required'); null if absent. Use null for any missing field. Also return legacy field 'ids' as an array of any FULL 13-digit numbers found. If nothing found, return {\"records\":[],\"ids\":[]}. No prose, no markdown.",
          },
          {
            role: "user",
            content: [
